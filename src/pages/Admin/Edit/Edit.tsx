...
import { getAllTagsAndCategories, getBlogDataForEdit, getImageUrl, updateOrAddBlog } from '@/services/adminService';
import { BlogCategory, BlogTag, GalleryImage, UpdateOrAddBlogRequest } from '@/types';
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setErrors({
          submit: uploadError instanceof Error ? `上传失败: ${uploadError.message}` : '上传文章内容失败，请稍后重试',
        });
        setSubmitLoading(false);
        return;
      }
      ...
      const requestData: UpdateOrAddBlogRequest = {
        ...
        // 处理分类信息
        category_id: category?.category_id && !category.category_id.startsWith('cat_') ? category.category_id : '', // 如果是新分类或临时ID，则category_id为空
        category: {
  ...
  return (
    <div className='edit-page'>
      <div className='edit-container'>
        ...
        {showCachePrompt && (
          <div className='cache-prompt'>
            ...
            <div className='cache-actions'>
              <button type='button' onClick={restoreFromCache} className='restore-button'>恢复草稿</button>
              <button type='button' onClick={discardCache} className='discard-button'>丢弃草稿</button>
            </div>
          </div>
        )}
        ...
        {loading ? <div className='loading-container'>正在加载文章数据...</div> : (
                  ...
                  <div className='tags-input-wrapper'>
                    {tags.map((tag) => (
                      <div key={tag.tag_id} className='tag-item'>
                        ...
                        <button type='button' className='remove-btn' onClick={() => removeTag(tag)}>
                          <FiX />
                        </button>
                      </div>
                    ))}
