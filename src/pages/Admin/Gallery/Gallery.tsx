...
import { FiCode, FiEdit, FiFileText, FiLoader, FiPlus, FiSearch, FiTrash2 } from 'react-icons/fi';
import './Gallery.scss';
// 导入布局上下文
...
const ContextMenu: React.FC<ContextMenuProps> = React.memo(({
  ...
  return createPortal(
    ...
    >
      <ul>
        <li onClick={onRename}>
          <FiEdit className='menu-icon' /> 重命名
        </li>
        <li onClick={onDelete}>
          <FiTrash2 className='menu-icon' /> 删除
        </li>
        <li onClick={onCopyHTML}>
          <FiCode className='menu-icon' /> 复制 HTML 代码
        </li>
        <li onClick={onCopyMarkdown}>
          <FiFileText className='menu-icon' /> 复制 Markdown 代码
        </li>
      </ul>
    </div>,
    document.body,
...
const Gallery: React.FC = () => {
  ...
  const filteredImages = useMemo(() => {
    ...
    return allImages.filter((img) => img.name.toLowerCase().includes(lowerCaseSearchTerm));
  ...
  const handleRenameComplete = useCallback(async (imageId: string, newName: string) => {
    ...
    try {
      ...
      if (response.code === 200) {
        // 更新本地状态
        setAllImages((prevImages) => prevImages.map((img) => img.id === imageId ? { ...img, name: newName } : img));
      } else {
  ...
  return (
    <div className='gallery-container' ref={galleryContainerRef}>
      <div className='search-box'>
        ...
        />
        <button type='button' className='add-image-btn' onClick={openUploadModal}>
