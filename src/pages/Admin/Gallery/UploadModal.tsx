...
const UploadModal: React.FC<UploadModalProps> = React.memo(({ visible, onClose, onImagesUploaded }) => {
  ...
  const uploadImageToOSS = useCallback(
    async (file: UploadFile, index: number): Promise<{ success: boolean; fileName: string } | false> => {
      try {
                        ...
                        {file.isCompressed
                          ? (
                            <div className='compression-info'>
                              <div className='compression-status success'>
                                <FiCheckCircle />
                                <span>
                                  {file.isUploaded ? '已上传' : file.uploadError ? '上传失败' : '已压缩'}
                                </span>
                              </div>
                          ...
                          : (
                            <div className='compression-status'>
                              {file.isCompressing ? <span>压缩中...</span> : <span>等待压缩</span>}
                            </div>
                          )}
                      </div>
                      <button
