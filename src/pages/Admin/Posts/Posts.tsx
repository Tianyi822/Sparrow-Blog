  ...
  const handleTogglePin = useCallback(async (id: string) => {
    try {
      ...
      if (response.code === 200) {
        // 更新本地状态
        setPosts((prevPosts) =>
          prevPosts.map((post) => post.blog_id === id ? { ...post, blog_is_top: !post.blog_is_top } : post)
        );
      } else {
        // 显示错误消息
  ...
  const handleToggleStatus = useCallback(async (id: string) => {
    try {
      ...
      if (response.code === 200) {
        // 更新本地状态
        setPosts((prevPosts) =>
          prevPosts.map((post) => post.blog_id === id ? { ...post, blog_state: !post.blog_state } : post)
        );
      } else {
        // 显示错误消息
    ...
    return (
      <div className='search-container'>
        <div className='search-input-wrapper'>
          ...
          />
          {searchTitle && (
            <button type='button' className='clear-search' onClick={() => setSearchTitle('')}>
              <FiX />
            </button>
          )}
        </div>
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (activeTagPopup !== null) {
        ...
        if (
          popupElement && !popupElement.contains(clickedElement) &&
  ...
  return (
    <div className='admin-posts'>
                    ...
                    <div className='tag-container'>
                      {post.tags.length === 0
                        ...
                        : (
                          <>
                            {getTagLimitCount() > 0 &&
                              post.tags.slice(0, getTagLimitCount()).map((
                                tag: { tag_id: string; tag_name: string },
                              ) => (
                                <span key={tag.tag_id} className='tag-badge' title={tag.tag_name}>{tag.tag_name}</span>
                              ))}
                            <span
                ...
                <td className='action-column'>
                  <div className='action-buttons'>
                    <button
                      ...
                      onClick={() =>
                        handleToggleStatus(post.blog_id)}
                    >
                    ...
                    <button
                      ...
                      title='删除'
                      onClick={() => handleDeletePost(post.blog_id, post.title)}
                    >
                      <FiTrash2 />
                    </button>
                  </div>
      ...
      {activeTagPopup !== null && popupPosition && (
        ...
        >
          {posts.find((post) => post.blog_id === activeTagPopup)?.tags.slice(getTagLimitCount()).map((
            tag: { tag_id: string; tag_name: string },
