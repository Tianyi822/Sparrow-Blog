...
import { FiEdit, FiTrash2, FiArrowUp, FiEye, FiEyeOff, FiChevronLeft, FiChevronRight, FiSearch, FiX, FiSettings } from 'react-icons/fi';
import './Posts.scss';
...
const mockPosts: Post[] = Array.from({ length: 100 }, (_, index) => {
  // 生成随机数量的标签，有些文章会超过5个
  const tagCount = Math.floor(Math.random() * 8) + 1; // 1到8个标签
  const tags = [];
            ...
            {getCurrentPagePosts().map(post => (
              <tr key={post.id} className={post.pinned ? 'pinned-row' : ''}>
                ...
                <td className="tags-column">
                  ...
                  {post.tags.length > 5 && (
                    <span className="more-tags">+{post.tags.length - 5}</span>
                  )}
                </td>
