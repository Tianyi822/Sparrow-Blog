...
const Home: React.FC = () => {
  ...
  // 获取用户头像URL
  const avatarImageUrl = homeData?.avatar_image ? getImageUrl(homeData.avatar_image) : '';
    ...
    if (homeData && homeData.blogs) {
      try {
        ...
        const formattedBlogs: BlogData[] = displayBlogs.map((blog: BlogInfo) => {
          // 安全地访问可能为空的属性
          const categoryId = blog.category && blog.category.category_id ? blog.category.category_id.toString() : '';
          const categoryName = blog.category && blog.category.category_name ? blog.category.category_name : '';
        ...
        <div className='blog-content'>
          {activeFilterName && (
            <div className='active-filter'>
              ...
              <button type='button' className='clear-filter' onClick={handleClearFilter}>清除筛选</button>
            </div>
          )}
