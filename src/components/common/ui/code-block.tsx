...
export const CodeBlock = ({
  ...
  const activeCodeData = useMemo(() => {
    // 确保代码是字符串类型
    const codeString = tabsExist ? String(tabs[activeTab]?.code || '') : String(code || '');

    const activeLang = tabsExist ? tabs[activeTab]?.language || language : language;

    const activeHighlights = tabsExist ? tabs[activeTab]?.highlightLines || [] : highlightLines;
  ...
  const getLineProps = useCallback((lineNumber: number) => ({
    style: {
      display: 'block',
      backgroundColor: activeCodeData.highlightLines.includes(lineNumber) ? 'rgba(255,255,255,0.1)' : 'transparent',
    },
